...
impl ResourceTrait for Vpn {
    ...
    fn gauge_hour(&self) -> Result<GenericGauge<AtomicF64>, prometheus::Error> {
        let vpn_gauge_hour_opts = Opts::new("vpn_price_hour", "Vpn price by hour")
            ...
            .const_label("resource_type", "Vpn".to_string());
        let vpn_gauge_hour = Gauge::with_opts(vpn_gauge_hour_opts).or_else(|e| Err(e));
        vpn_gauge_hour
    }
    ...
    fn gauge_month(&self) -> Result<GenericGauge<AtomicF64>, prometheus::Error> {
        let vpn_gauge_month_opts = Opts::new("vpn_price_month", "Vpn price by month")
            ...
            .const_label("resource_id", self.resource_id.as_ref().unwrap())
            .const_label("resource_type", "Vpn".to_string());
        let vpn_gauge_month = Gauge::with_opts(vpn_gauge_month_opts).or_else(|e| Err(e));
        vpn_gauge_month
    }
}

pub struct VpnMetrics {
    pub vpn_price_per_hours: GenericGauge<AtomicF64>,
    pub vpn_price_per_months: GenericGauge<AtomicF64>,
}
impl ResourceMetricsTrait for VpnMetrics {
    fn register(&self, registry: Registry) -> Result<Registry, prometheus::Error> {
        registry
            .register(Box::new(self.vpn_price_per_hours.clone()))
            .or_else(|e| Err(e))?;
        registry
            .register(Box::new(self.vpn_price_per_months.clone()))
            .or_else(|e| Err(e))?;
        Ok(registry)
    }
}
